...
echo -e "${BLUE}=====================================================================${NC}"
echo -e "${BLUE}E-commerce Platform - AWS Deployment${NC}"
echo -e "${BLUE}=====================================================================${NC}"
echo ""
...
if [ ! -f "terraform.tfvars" ]; then
  ...
  read -p "Coralogix Token (cxtp_...): " CX_TOKEN
  read -p "Your IP address for SSH (x.x.x.x/32): " SSH_IP
  read -p "Coralogix RUM Public Key: " CX_RUM_KEY
  
  # Create terraform.tfvars
  cat > terraform.tfvars <<EOF
# Auto-generated by deploy-aws.sh for E-commerce Platform
# Created: $(date)

coralogix_token     = "$CX_TOKEN"
openai_api_key      = "not-used"
cx_rum_public_key   = "$CX_RUM_KEY"
allowed_ssh_cidr    = "$SSH_IP"

# E-commerce Platform Configuration
coralogix_app_name  = "ecommerce-platform"
coralogix_subsystem = "ecommerce-services"
...
echo -e "  • VPC with public subnet"
echo -e "  • Security group (SSH, HTTP, HTTPS)"
echo -e "  • IAM role for Coralogix integration"
echo -e "  • EC2 t3.small instance"
echo -e "  • 30GB gp3 EBS volume"
echo -e "  • Elastic IP"
echo ""
...
SSH_KEY_PATH="$HOME/.ssh/ecommerce-platform-key.pem"
terraform output -raw ssh_private_key > "$SSH_KEY_PATH"
chmod 600 "$SSH_KEY_PATH"

echo -e "${GREEN}✓ SSH key saved to: $SSH_KEY_PATH${NC}"
...
echo -e "  Load Generator: ${GREEN}http://$PUBLIC_IP:8010${NC}"
echo ""
echo -e "${BLUE}SSH Access:${NC}"
echo -e "  ${GREEN}ssh -i ~/.ssh/ecommerce-platform-key.pem ubuntu@$PUBLIC_IP${NC}"
echo ""
echo -e "${YELLOW}⏱️  Bootstrap Status:${NC}"
echo -e "The instance is currently:"
echo -e "  1. Installing Docker and K3s"
echo -e "  2. Building Docker images for 8 microservices"
echo -e "  3. Deploying PostgreSQL, Redis, and services"
echo -e "  4. Installing Coralogix OpenTelemetry Collector"
echo -e ""
echo -e "This takes approximately ${YELLOW}10-15 minutes${NC}."
echo ""
echo -e "${BLUE}Monitor bootstrap progress:${NC}"
echo -e "  ${GREEN}ssh -i ~/.ssh/ecommerce-platform-key.pem ubuntu@$PUBLIC_IP 'tail -f /var/log/ecommerce-bootstrap.log'${NC}"
echo ""
echo -e "${BLUE}Generate traffic once ready:${NC}"
echo -e "  ${GREEN}curl -X POST http://$PUBLIC_IP:8010/admin/generate-traffic -H 'Content-Type: application/json' -d '{\"duration_seconds\": 60, \"requests_per_minute\": 30}'${NC}"
echo ""
echo -e "${BLUE}View in Coralogix (EU2):${NC}"
echo -e "  Application: ${GREEN}ecommerce-platform${NC}"
echo -e "  https://eu2.coralogix.com"
echo ""
